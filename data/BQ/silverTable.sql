...
CREATE TABLE IF NOT EXISTS `sri-project-455613.silver.customers`
(
    customer_id INT64,
    name STRING,
    email STRING,
...
MERGE INTO  `sri-project-455613.silver.customers` target
USING 
  (SELECT DISTINCT
    *, 
    CASE 
    ...
    True as is_active
  FROM `sri-project-455613.bronze.customers`) source
...
CREATE TABLE IF NOT EXISTS `sri-project-455613.silver.orders`
(
    order_id INT64,
    customer_id INT64,
    order_date STRING,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `sri-project-455613.silver.orders` target
USING 
  (SELECT DISTINCT
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `sri-project-455613.bronze.orders`) source
...
CREATE TABLE IF NOT EXISTS `sri-project-455613.silver.order_items`
(
    order_item_id INT64,
    order_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `sri-project-455613.silver.order_items` target
USING 
  (SELECT DISTINCT
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `sri-project-455613.bronze.order_items`) source
...
CREATE TABLE IF NOT EXISTS `sri-project-455613.silver.categories`
(
    category_id INT64,
    name STRING,
    updated_at STRING,
...
TRUNCATE TABLE `sri-project-455613.silver.categories`;

--Step 3: Insert New or Updated Records
INSERT INTO `sri-project-455613.silver.categories`
SELECT 
  *,
  CASE 
...
FROM `sri-project-455613.bronze.categories`;

--Step 1: Create the products Table in the Silver Layer
CREATE TABLE IF NOT EXISTS `sri-project-455613.silver.products`
(
  product_id INT64,
  name STRING,
  category_id INT64,
...
--Step 2: Truncate table
TRUNCATE TABLE `sri-project-455613.silver.products`;

--Step 3: Insert New or Updated Records
INSERT INTO `sri-project-455613.silver.products`
SELECT 
  *,
  CASE 
    WHEN category_id IS NULL OR name IS NULL THEN TRUE
...
FROM `sri-project-455613.bronze.products`;
-------------------------------------------------------------------------------------------------------------
--Step 1: Create the product_supplier Table in the Silver Layer
CREATE TABLE IF NOT EXISTS `sri-project-455613.silver.product_suppliers`
(
    supplier_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `sri-project-455613.silver.product_suppliers` target
USING 
  (SELECT 
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `sri-project-455613.bronze.product_suppliers`) source
...
CREATE TABLE IF NOT EXISTS `sri-project-455613.silver.suppliers`
(
  supplier_id INT64,
...
--Step 2: Truncate table
TRUNCATE TABLE `sri-project-455613.silver.suppliers`;

--Step 3: Insert New or Updated Records
INSERT INTO `sri-project-455613.silver.suppliers`
SELECT 
  *,
  CASE 
...
FROM `sri-project-455613.bronze.suppliers`;

-------------------------------------------------------------------------------------------------------------

--Step 1: Create the customer_reviews Table in the Silver Layer
CREATE TABLE IF NOT EXISTS `sri-project-455613.silver.customer_reviews`
(
    id STRING,
    customer_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `sri-project-455613.silver.customer_reviews` target
USING 
  (SELECT 
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `sri-project-455613.bronze.customer_reviews`) source
